...
def render_report(summary: dict) -> str:
    ...
    lines = [
        ...
        f"- Resources with native `id`: {dataset['with_id_resource_count']}",
    ...
    for scenario in summary["scenarios"]:
        lines.append(f"- `{scenario['label']}` ({scenario['category']}): `{scenario['path']}`")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Render a markdown report from the PokeAPI benchmark summary JSON."
    )
