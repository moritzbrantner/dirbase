...
fn sql_happy_paths_cover_select_projection_and_pagination() {
    ...
    assert!(
        select_all.starts_with("HTTP/1.1 200 OK\r\n"),
        "{select_all}"
    );
...
fn sql_error_paths_cover_non_select_unsupported_and_unknowns() {
    ...
    let non_select = http_post_json(
        "127.0.0.1:3021",
        "/sql",
        serde_json::json!({"query": "DELETE FROM users"}),
    );
    assert!(
        non_select.starts_with("HTTP/1.1 400 Bad Request\r\n"),
        "{non_select}"
    );
    ...
    let unsupported = http_get(
        "127.0.0.1:3021",
        "/sql?q=SELECT%20DISTINCT%20name%20FROM%20users%20LIMIT%201",
    );
    assert!(
    ...
    assert!(
        unknown_table.starts_with("HTTP/1.1 404 Not Found\r\n"),
        "{unknown_table}"
    );
    ...
    let unknown_column = http_get(
        "127.0.0.1:3021",
        "/sql?q=SELECT%20email%20FROM%20users%20LIMIT%201",
    );
    assert!(
        unknown_column.starts_with("HTTP/1.1 400 Bad Request\r\n"),
        "{unknown_column}"
    );
...
fn export_sql_is_stable_and_escapes_and_handles_nulls_without_schema() {
    ...
    let z_idx = body
        .find("-- Resource: z_projects")
        .expect("z_projects block");
...
fn readonly_mode_allows_sql_and_export_and_blocks_mutation_sql() {
    ...
    assert!(
        parse_http_body(&export).contains("INSERT INTO \"users\""),
        "{export}"
    );
    ...
    assert!(
        mutation_sql.starts_with("HTTP/1.1 400 Bad Request\r\n"),
        "{mutation_sql}"
    );
